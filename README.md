## 👋 Hi, I'm Oliwia!  
Marketing Analyst with **4+ years** in digital marketing, customer service and data analysis. Aspiring **Systems Analyst** – bridging business needs with tech solutions.

## 🛠️ Technical Skills

| Category | Tools |
|----------|-------|
| **Analytics** | Python, SQL, **Power BI**, Excel, Google Analytics, SPSS |
| **APIs & Docs** | JSON/XML, **Postman**, JavaScript/CSS |
| **Process Modeling** | **UML/Draw.io**, Agile/Scrum/**Jira** |
| **Design & PM** | **Figma**, WordPress, ClickUp |

## 🔥 Featured Projects

| Project | Description | Tech Stack | Link |
|---------|-------------|------------|------|
| 🏠 **House Price Prediction** | ML model predicting real estate prices using data science techniques | Python/ML | [GitHub](https://github.com/oolender/house_price_prediction) |
| 📈 **Stock Revenue Analysis** | Financial data analysis with revenue insights & visualizations | Python/Power BI | [GitHub](https://github.com/oolender/stock_revenue_analysis) |

## 🌐 Let's connect!

[LinkedIn](https://linkedin.com/oliwia-olender)
[GitHub](https://github.com/oolender)


---
**🔔 Open to Systems Analyst roles, data projects, or marketing tech collaborations! 🚀**
